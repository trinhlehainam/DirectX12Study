...
cbuffer BoardConstant : register(b0)
{
	int g_enableShowDebug;
	int g_enableBloom;
	int g_enableDoF;
	float padding0;
}
...
float4 boardPS(BoardOutput input) : SV_TARGET
{
	...
	if (g_enableDoF)
		...
		{
			return g_enableBloom ? focusColor + brightColor : focusColor;
		}
	
	if (color.a > 0.0f)
	{
		return g_enableBloom ? color + brightColor : color;
	}
